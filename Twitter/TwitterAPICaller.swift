...
//using Auth1Manager pod this class helps us login to twitter using OAuth
class TwitterAPICaller: BDBOAuth1SessionManager {
    ...
    func getUserDictionariesRequest(success: @escaping (NSDictionary) -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/users/show.json"
        let params:[String:Any] = ["screen_name": "GalacticWandere"]
        TwitterAPICaller.client?.get(url, parameters: params, success: { (task: URLSessionDataTask, response: Any?) in
            success(response as! NSDictionary)
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    //posts tweet to API
    func postTweet(with tweet: String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func favoriteTweet(with tweetID: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func unFavoriteTweet(with tweetID: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func reTweet(with tweetID: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
